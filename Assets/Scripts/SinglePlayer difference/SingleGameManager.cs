...
public class SingleGameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject.Instantiate(playerModel, new Vector3(leftspawn.transform.position.x, leftspawn.transform.position.y, leftspawn.transform.position.z), Quaternion.identity);

        rightspawn = GameObject.FindGameObjectWithTag("RightSpawn");
        GameObject.Instantiate(playerModel, new Vector3(rightspawn.transform.position.x, rightspawn.transform.position.y, rightspawn.transform.position.z), Quaternion.identity);

        Camera[] camList = FindObjectsOfType<Camera>();

        AudioListener[] listenerList = FindObjectsOfType<AudioListener>();
        ...
        foreach (AudioListener listener in listenerList)
        {
            listener.enabled = false;
        }

        camList[1].enabled = true;

        listenerList[1].enabled = true;
    ...
    public void SetSyncedCopy(bool value) 
    {
        syncedCopy = value;
    }
    ...
    void Update()
    {
        if (!ready)
        {
            ...
            if (synced && syncedCopy)
            {
                ...
                Debug.Log("players spawned and base loaded");
                SceneToLoad = 2;

                SingleGameManager.instance.LoadPlayerSecondary(Me, Enemy);
                SingleGameManager.instance.LoadPlayerSecondary(Enemy, Me);
                ...
                SingleInteraction.instance.SetPlayer1(Me);
                SingleInteraction.instance.SetPlayer2(Enemy);
                ...
                leftBorderPos = GameObject.FindGameObjectWithTag("LeftSpawn").transform.position.x;

                rightBorderPos = GameObject.FindGameObjectWithTag("RightSpawn").transform.position.x;
    ...
    public void LoadPlayerBase(GameProcess player, bool meOrEnemy)
    {
        ...
        //Debug.Log(player.stunModif + " ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss" + " " + DataController.GetValue<int>("TotalModifierStunChance" + MineOrOther));
    ...
    void WinAndLoseAnimation()
    {
        ...
        LocalNumberForLoseAnimName = (int)Time.time % 3;
