...
public class SinglePlayerCamera : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (flag1 == true)
        {
            if (temp1 != null)
            {
                ScaledWithCameraFront = GameObject.FindGameObjectWithTag("BtnsFront");
                ScaledWithCameraBack = GameObject.FindGameObjectWithTag("BtnsBack");
            ...
            {
                temp1 = GameObject.FindGameObjectWithTag("Player");
            }
        }
        if (targets.Count == 0)
            return;
